 ...
 */
static void read_from_mpl(float* pitch, float* roll)
{
    long msg, data[9];
    int8_t accuracy;
...
int main(void)
{
    ...
    // maxNum: record the max value of block
    // score: record the score of player
    uint16_t maxNum = 0, score = 0;

    // g[4][4]: record the blocks in the game
    // v[4][4]: record whether the block is used or not
    uint16_t g[4][4], v[4][4];

    // havewin: flag to record winning game
    // havelose: flag to record losing game
    uint8_t havewin = 0, havelose = 0;

    // record the moving direction
    uint8_t direction = 0;

    // save the `pitch` and `roll` read from sensor
    ...
    while(1){
        ...
        if (new_data) 
        {
             ...
             */
            read_from_mpl(&pitch, &roll);

            // printf("%f\n%f\n", pitch, roll);

            // determine the direction by sensor data
            direction = EnsureDirection(pitch, roll);

            // press K2 to confirm the moving direction
            if( Key_Scan(KEY2_GPIO_PORT, KEY2_GPIO_PIN) == KEY_ON)
            {   
                MoveStep(g, v, &maxNum, &score, direction, &havewin, &havelose);
            }
